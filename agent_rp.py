...
image_height, image_width = 50, 80 #TODO: change to 72
merged_model = []
...
class Agent(object):
    def __init__(self, discount, level, algorithm, prioritized_experience, max_memory, exploration_policy,
        ...
        # reward prediction
        self.reward_prediction = True

        # initialization
        self.environment = Environment(level=level, combine_actions=combine_actions, visible=visible)
        self.memory_zero = ExperienceReplay(max_memory=max_memory, prioritized=prioritized_experience, store_episodes=(max_action_sequence_length>1))
        self.memory_non_zero = ExperienceReplay(max_memory=max_memory, prioritized=prioritized_experience, store_episodes=(max_action_sequence_length>1))
        ...
        if self.reward_prediction:
            self.reward_prediction_model = self.create_network_rp()

    def create_network_rp(self):
        '''
        reward prediction network
        output is 3 class: neg, zero, pos
        '''
        print "reward prediction constructed"
        input_tensor = self.online_network.input
        flatten = self.online_network.layers[-3].output
        fc1_rp = Dense(256,name='fc1_rp')(flatten)
        output_layer = Dense(3, name='output_rp')(fc1_rp)
        model = Model(input=input_tensor, output=output_layer)
    ...
    def get_inputs_and_targets_for_sequence(self, minibatch):
        ...
        for idx, transition_list, game_over, sample_weight in minibatch:
            ...
            if self.memory_non_zero.prioritized:
                self.memory_non_zero.update_transition_priority(idx, np.abs(TD_error))
    ...
    def store_next_state(self, preprocessed_next, reward, game_over, action_idx):
        ...
        # store transition
        if reward != 0:
            self.memory_non_zero.remember(Transition(preprocessed_curr, action_idx, reward, preprocessed_next), game_over) # stored as np array
        else:
            self.memory_zero.remember(Transition(preprocessed_curr, action_idx, reward, preprocessed_next), game_over) # stored as np array
    ...
    def get_inputs_and_targets_rp(self, minibatch):
        """Given a minibatch, extract the inputs and targets for the training reward prediction model

        :param minibatch: the minibatch to train on
        :return: the inputs, targets
        """
        
        targets = list()
        
        inputs = list()
        for idx, transition_list, _, _ in minibatch:
            transition = transition_list[0]
            inputs.append(transition.preprocessed_curr[0])

            target = np.array([0]*3)
            curr_reward = transition.reward
            if curr_reward > 0:
                target[2] = 1
            elif curr_reward == 0:
                target[1] = 1
    ...
    def train(self):
        ...
        if len(self.memory_non_zero.memory)!= 0:
            minibatch = self.memory_non_zero.sample_minibatch(self.batch_size)
        else:
            minibatch = self.memory_zero.sample_minibatch(self.batch_size)

        
        # print self.reward_prediction
        if self.reward_prediction and len(self.memory_non_zero.memory)!= 0 and  len(self.memory_zero.memory)!= 0:
            # print ">>learning reward prediction\n"
            minibatch_zero = self.memory_zero.sample_minibatch(self.batch_size/2)
            minibatch_reward = minibatch_zero + minibatch[:self.batch_size/2]
            inputs_r, targets_r = self.get_inputs_and_targets_rp(minibatch_reward)
            # set_trace()
            self.reward_prediction_model.train_on_batch(inputs_r, targets_r)
